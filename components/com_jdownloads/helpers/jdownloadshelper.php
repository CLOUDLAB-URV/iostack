    ...
    public static function updateLog($type, $files, $upload_data)
    {
        ...
        if ($type == 1){
            foreach ($files as $file){
                ...
                if ($file->extern_file != ''){
                    $db->setQuery("INSERT INTO #__jdownloads_logs (type, log_file_id, log_file_size, log_file_name, log_title, log_ip, log_datetime, log_user) VALUES ( '".$type."', '".$file->file_id."', '".$filesize."', '".$file->extern_file."', '".$db->getEscaped($file->file_title)."',  '".$ip."', '".$date."', '".$user->get('id')."')");
                } else {
                    $db->setQuery("INSERT INTO #__jdownloads_logs (type, log_file_id, log_file_size, log_file_name, log_title, log_ip, log_datetime, log_user) VALUES ( '".$type."', '".$file->file_id."', '".$filesize."', '".$db->getEscaped($file->url_download)."', '".$db->getEscaped($file->file_title)."',  '".$ip."', '".$date."', '".$user->get('id')."')");
                }    
                $db->query();
            }
        } else {
            if ($type = 2){
                ...
                $db->setQuery("INSERT INTO #__jdownloads_logs (type, log_file_id, log_file_size, log_file_name, log_title, log_ip, log_datetime, log_user) VALUES ( '".$type."', '".$upload_data->file_id."', '".$filesize."', '".$db->getEscaped($upload_data->url_download)."', '".$db->getEscaped($upload_data->file_title)."',  '".$ip."', '".$date."', '".$user->get('id')."')");
                $db->query();                
            }
