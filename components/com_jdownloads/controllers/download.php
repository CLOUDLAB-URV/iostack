...
class jdownloadsControllerDownload extends JControllerForm
{
	...
	protected function allowAdd($data = array())
	{
		...
		$categoryId	= JArrayHelper::getValue($data, 'cat_id', JRequest::getInt('catid'), 'int');
		$allow		= null;
    ...
    public function send()
    {
        ...
        // get request data
        $cat_id     = $db->escape(JRequest::getVar('catid', 0, '', 'int'));
        $file_id    = $db->escape(JRequest::getVar('id', 0, '', 'int'));
        $mirror     = $db->escape(JRequest::getVar('m', 0, '', 'int'));
        $files_list = $db->escape(JRequest::getVar('list', '', '', 'string'));
        $zip_file   = $db->escape(JRequest::getVar('user', 0, '', 'cmd'));
